...
const About = () => {
  ...
  const { elementRef: storyRef, isVisible: storyVisible } = useScrollAnimation({ threshold: 0.1 });
  ...
  return (
    <>
      ...
      <main className="flex-1">
        <section ref={heroRef as React.RefObject<HTMLElement>} className="relative overflow-hidden bg-muted py-20">
          <div className="w-full absolute inset-0 h-full">
            <Meteors number={30} />
          </div>
          <div className={`container mx-auto px-4 text-center relative z-10 ${heroVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
        ...
        <section className="py-16 md:py-24">
          <div className="container mx-auto px-4 max-w-4xl">
            <div ref={storyRef as React.RefObject<HTMLDivElement>} className={`prose prose-lg mx-auto mb-12 ${storyVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
            ...
            <div ref={valuesRef as React.RefObject<HTMLDivElement>} className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {valueCards.map((card, index) => {
                ...
                return (
                  <Card 
                    key={index} 
                    className={`shadow-soft ${valuesVisible ? 'animate-scale-in' : 'opacity-0'} animate-delay-${index * 100}`}
                  >
