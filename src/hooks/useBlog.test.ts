// Tests: verify that useBlog exposes correctly localized blog metadata for both supported languages.
/**
 * File overview: src/hooks/useBlog.test.ts
 *
...
describe("useBlog", () => {
  ...
  it("should return localized blog posts in Spanish", () => {
    ...
    expect(result.current.blogPosts).toHaveLength(6);
    expect(result.current.blogPosts[0].title).toBe(
      "Redes para Pymes de Florida: Lo Mínimo que Funciona"
    );
  });
  ...
  it("should memoize results for the same language", () => {
    const { result, rerender } = renderHook(({ lang }) => useBlog(lang), {
